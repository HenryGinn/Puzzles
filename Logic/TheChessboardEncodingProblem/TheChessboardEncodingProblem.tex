...
\begin{figure}[H]
	\begin{center}
		...
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with the right four columns highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_1.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid1}
		\end{subfigure}\hfill
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with columns 3, 4, 7, and 8 highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_2.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid2}
		\end{subfigure}\hfill
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with columns 2, 4, 6, and 8 highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_3.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid3}
		\end{subfigure}\hfill\vspace{6mm}
			\\
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with the bottom four rows highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_4.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid4}
		\end{subfigure}\hfill
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with rows 3, 4, 7, and 8 highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_5.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid5}
		\end{subfigure}\hfill
		\begin{subfigure}{0.3\linewidth}
			\begin{tikzpicture}[scale=0.6, alt={An $8 \times 8$ grid with rows 2, 4, 6, and 8 highlighted red.}]
				\input{Logic/TheChessboardEncodingProblem/Chessboard_6.tikz}
			\end{tikzpicture}
			\caption{}
			\label{fig:TheChessboardEncodingProblem_Grid6}
		\end{subfigure}
	\end{center}
...
In the example shown in figure~\ref{fig:TheChessboardEncodingProblem_Example}, the key has been hidden in the cell in the third row and seventh column. Using the first three digits to encode the column number and the last three digits to encode the row number, this is encoded as 111011. In the red highlighted region in figure~\ref{fig:TheChessboardEncodingProblem_Grid1} there are 19 heads which is equal to 1 modulo 2. This matches the first digit in the number we want to encode which means we want to preserve the parity of heads in this region. Therefore we will flip a coin outside the red highlighted region, that is, on the left hand side. We can do a similar process for the other regions and the results of this are given in table~\ref{tab:TheChessboardEncodingProblem_ExampleParity}

\begin{figure}[H]
	\centering
	\begin{tikzpicture}[scale=1, alt={An $8 \times 8$ grid with coins on each square, randomly either heads of tales. The square in the third row and seventh column is highlighted in red.}]
		\input{Logic/TheChessboardEncodingProblem/ChessboardExample.tikz}
	\end{tikzpicture}
...
\begin{table}[H]
	\centering
	\caption{An example of how to determine which regions should have their parity flipped}
	...
	\begin{tabular}{cccccc}
		...
		\myhline
		\tableinput{Logic/TheChessboardEncodingProblem/ChessboardExampleTable.tex}
		\myhline
	\end{tabular}
\end{table}
